...
| 内部缝 | `memory/llm.py` / `embed.py` / `store.py` | ✅ OpenRouter(claude-sonnet-4.6,key 从 gitignored `.env`)/ MiniLM 缓存单例+手算 cosine / 进程内 store |
...
- **llm 缝实测**:OpenRouter + `claude-sonnet-4.6` + `response_format=json_object` 返回干净 JSON ✅(修了 `.env` 的引号包裹 + 模型 slug)。
...
| 项 | 结果 |
|---|---|
| extract 路径 | **real-extract**;stem=`JQ has a severe contact allergy to`,target=`nickel buckles`(LLM 自动分解)|
| PRE(raw, RAG off) | `nickel, which is a common allergen…` —— target **absent**(before/after 干净)|
| consolidate | `n_written=1`,edit latency **4.47s**,**buffer drained**(no-double-existence)|
| **CHAT must-pass(RAG off)** | `nickel buckles…` **token-EM=1.0 ✅** —— 答案来自**权重**,非 context |
| RAW sanity(RAG off) | hit=True |
| paraphrase bonus(`What is JQ allergic to?`,chat, RAG off) | hit=True(generalization 此条也命中)|
| 显存峰值 / 模型加载 | **16.2 GB** / 62.6 s |
| torch 复核 | start/end 均 `2.8.0+cu128 True` |

> 注:生成出现 `buckles buckles…` 重复 —— greedy 解码在答出 target 后继续吐 token 所致(cosmetic);target 已命中、token-EM=1.0,hero 证明成立。
