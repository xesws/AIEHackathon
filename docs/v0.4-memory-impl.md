...
### 多样本 eval(`spikes/spike_v04_samples_eval.py`,5 个 type-A 顺序固化进同一 codebook,RAG OFF)
- **reliability 5/5**(每条 edit stem → target,token-EM=1.0)、**generalization 7/10**(paraphrase 问句)、**locality OK 无泄漏**("capital of France" → "Paris",未误伤)。
- n_written=5/5(全 distinct → new);5 edit/23.05s;峰值 16.3GB;torch 不动。
- 结论:**顺序固化进同一 codebook 不互相干扰**(reliability 全过 + locality 干净)—— Engram 持续固化设计成立的关键证据。
