...
class ComprehendEditDataset(BaseDataset):
    def __init__(self, data_dir: str, size: typing.Optional[int] = None, config=None, *args, **kwargs):
        ...
        self.annotation = [
            json.loads(q) for q in open(os.path.join(data_dir, "ComprehendEdit_{}.json".format(self.mode)), "r")
        ][0]
        ...
        if os.path.exists(path):
            ...
            for i, record in enumerate(self.annotation):
                ...
                item = {
                    ...
                    "cond": "{} >> {} || {}".format("", record["answer"], record["question"]),
            ...
            if os.path.exists(path):
                ...
                for step, sample in enumerate(data):
                    ...
                    edit_inner["text_input"] = [
                        self.prompt[source].format(s) + t for s, t, source in zip(src, trg, sources)
                    ]
    ...
    def get_prompt_len(self, src, trg, sources, imgs=True):
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2":
            prompts_len = [
                len(self.tok.encode(self.prompt[source].format(s), add_special_tokens=False))
                for s, source in zip(src, sources)
            ]
            labels = self.tok(
                trg,
    ...
    def collate_fn(self, batch):
        ...
        edit_inner["rephrase_text_input"] = [
            self.prompt[source].format(r) + t for r, t, source in zip(rephrase, trg, sources)
        ]
        ...
        if self.mode != "train" and self.topk != -1 and self.test_:
            img_topk, txt_topk = [b["img_topk"][: self.topk] for b in batch], [
                b["txt_topk"][: self.topk] for b in batch
            ]
            edit_inner["img_topk"], edit_inner["txt_topk"] = [], []
            img_last_topk, txt_last_topk = [b["img_last_topk"][-self.topk :] for b in batch], [
                b["txt_last_topk"][-self.topk :] for b in batch
            ]
            ...
            img_topk, txt_topk = [b["ori_rt_img_topk"][: self.topk] for b in batch], [
                b["ori_rt_txt_topk"][: self.topk] for b in batch
            ]
            edit_inner["ori_rt_img_topk"], edit_inner["ori_rt_txt_topk"] = [], []
            img_last_topk, txt_last_topk = [b["ori_rt_img_last_topk"][-self.topk :] for b in batch], [
                b["ori_rt_txt_last_topk"][-self.topk :] for b in batch
            ]
