...
class PersonalityDataset(Dataset):
    ...
    def __init__(self, data_dir: str, size: typing.Optional[int] = None, config=None, *args, **kwargs):
        ...
        for case_idx, sample in enumerate(raw):
            target_per = (
                random.choice([0, 1, 2]) if "target_per" not in sample.keys() else self.per2id[sample["target_per"]]
    ...
    def collate_gpt_fn(self, batch):
        ...
        same_mask = torch.tensor(
            [i == o for i, o in zip(edit_toks["target_per"], edit_toks["all_per"])], device=self.config.device
        )
        batch = {
