    ...
    def process_csv_data(self, df):
        ...
        for _, row in df.iterrows():
            ...
            record = {
                ...
                # Efficacy test
                "efficacy": {"prompt": edit_prompt, "ground_truth": object_val},
                # Generalization tests (5 types)
                "generalization": {
                    "rephrase": {
                        "prompt": (
                            row.get("paraphrased_question") if pd.notna(row.get("paraphrased_question", None)) else None
                        ),
                        "ground_truth": object_val,
                    },
                    "yes": {
                    ...
                    "mcq": {
                        "prompt": (
                            row.get("multiple_choice_with_letters")
                            if pd.notna(row.get("multiple_choice_with_letters", None))
                            else None
                        ),
                        "ground_truth": (
                            row.get("multiple_choice_labels")
                            if pd.notna(row.get("multiple_choice_labels", None))
                            else None
                        ),
                    },
                    "reversed": {
                        "prompt": (
                            row.get("reversed_relation_question")
                            if pd.notna(row.get("reversed_relation_question", None))
                            else None
                        ),
