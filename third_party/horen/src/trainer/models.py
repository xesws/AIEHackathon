...
def get_model(config):
    if config.model_class == "BertClassifier":
        ...
        model = ModelClass.from_pretrained(
            config.model_name, trust_remote_code=True, device_map="auto" if config.model_parallel else None
        )
        ...
        if config.half:
            ...
            for pidx, p in enumerate(config.inner_params):
                ...
                if max_idx in comps or min_idx in comps:
                    ...
                    new_p = ".".join(comps)
                    LOG.info(f"Replacing config.inner_params[{pidx}] '{p}' -> '{new_p}'")
...
def get_tokenizer(config):
    ...
    tokenizer = getattr(transformers, config.tokenizer_class).from_pretrained(tok_name, cache_dir=scr())
