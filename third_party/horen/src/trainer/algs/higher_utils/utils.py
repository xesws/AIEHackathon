...
def _recursive_copy_and_cast(
    target: _typing.Union[list, tuple, dict, set, _torch.Tensor], device: _typing.Optional[_torch.device]
) -> _torch.Tensor:
    def map_fn(x):
...
def _get_param_mapping(
    module: _torch.nn.Module, seen: _typing.List[_torch.Tensor], mapping: _typing.List[int]
) -> _typing.List[int]:
...
def get_func_params(
    module: _torch.nn.Module, device: _typing.Optional[_torch.device] = None, safe_copy: bool = True
