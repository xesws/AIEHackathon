...
class SERAC(EditableModel):
    def __init__(
        ...
        if replacement is None:
            self.replacement_tok = getattr(transformers, config.tokenizer_class).from_pretrained(
                config.small_name, cache_dir="./hugging_cache"
            )
            ...
            if self.config.freeze_cntr:
                ...
                if config.model_class == "BertClassifier":
                    ...
                    self.replacement = getattr(transformers, config.model_class).from_pretrained(
                        config.small_name, cache_dir="./hugging_cache"
                    )
    ...
    def outer_parameters(self, grouped=False):
        ...
        extra_params = []
        if grouped:
            return [dict(params=model_params, lr=self.config.lr), dict(params=extra_params, lr=self.config.lr_lr)]
        else:
    ...
    def embedding_logsim_matrix(self, cls_ctxs, test_input_text):
        ...
        cls_main_input = self.classifier_tok(
            test_input_text, return_tensors="pt", max_length=512, truncation=True, padding=True
        ).to(self.config.device)
    ...
    def build_rep_input_tokens(self, kwargs, idxs, generation=False):
        ...
        if (
            ...
            pre = torch.full(
                (kwargs["labels"].shape[0], rep_kwargs["input_ids"].shape[-1] - kwargs["labels"].shape[-1]),
                -100,
        ...
        with torch.no_grad():
            if len(self.cache_inputs) == 0:
                if hasattr(self.model, "name_or_path") and (
                    ...
                    super_out = (
                        super()
                        .forward(*inputs, input_ids=kwargs["input_ids"], attention_mask=kwargs["attention_mask"])
                        .float()
                    )
                    # if 'labels' in kwargs.keys():
