...
from .malmen.util import get_module, get_shape, TracerDict, cross_entropy, kl_div, succ_ratios
...
class MALMEN(EditableModel):
    ...
    def __init__(self, model: nn.Module, config, model_constructor):
        ...
        self.net = nn.ModuleDict(
            {str(k): MALMENNet(*k, config.rank, config.n_blocks, v, config.lr) for k, v in shape_counter.items()}
        ).to(config.device)
    ...
    def predict_param_shifts(self, module_kv_map) -> Dict[str, torch.FloatTensor]:
        ...
        for module_idx, module_name in enumerate(self.config.inner_params):
            ...
            keys = torch.cat([module_kv_map[module_idx][idx]["keys"] for idx in range(len(module_kv_map[module_idx]))])
            values_grad = torch.cat(
                [module_kv_map[module_idx][idx]["values_grad"] for idx in range(len(module_kv_map[module_idx]))]
            )
        ...
        for module_idx, module_name in enumerate(self.config.inner_params):
            ...
            with torch.no_grad():
                mat = torch.linalg.solve(
                    keys.T @ keys + net.lamda(layer_idx).exp() * torch.eye(net.key_size, device=self.config.device),
                    module_grad,
                )
    ...
    def _inline_malmen_valid_log(self, step, stats, start_time, steps):
        ...
        LOG.info(f"Step {prog} edit_acc: {edit_acc} gen_acc: {gen_acc} loc_acc: {loc_acc}")
    ...
    def valid(self, config, loader, val_set, steps, log: bool = False):
        ...
        for val_step, batch in enumerate(loader):
            ...
            if (val_step + 1) % math.ceil(self.config.n_edits / self.config.batch_size) == 0 or val_step == steps - 1:
                ...
                for k, s in zip(["edit_inner", "edit_rephrase", "loc"], [edit_succs, gen_succs, loc_succs]):
                    for t in batch_dv[k]:
                        with torch.no_grad():
                            logits = self.model(input_ids=t["input_ids"], attention_mask=t["attention_mask"])["logits"]
