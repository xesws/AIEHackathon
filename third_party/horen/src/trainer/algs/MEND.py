...
from .patch import make_functional, monkeypatch as _make_functional
...
class MEND(EditableModel):
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        if self.config.shared:
            param_idx = lambda n, p: (
                self.shape_dict[self.get_shape(p)].index(n) if self.config.shared else None
            )  # noqa: E731
            transformed_factors = {
