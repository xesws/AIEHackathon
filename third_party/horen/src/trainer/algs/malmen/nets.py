...
class MALMENBlock(nn.Module):
    ...
    def forward(self, y: torch.FloatTensor, module_idx: torch.LongTensor) -> torch.FloatTensor:
...
class MALMENNet(nn.Module):
    ...
    def __init__(self, key_size: int, value_size: int, rank: int, n_blocks: int, n_modules: int, lr: float):
        super().__init__()
        self.key_size = key_size
        self.value_size = value_size

        self.normalizer = RunningMeanStd(key_size + value_size)
        self.blocks = nn.ModuleList([MALMENBlock(key_size + value_size, rank, n_modules) for _ in range(n_blocks)])
    ...
    def forward(
        self, keys: torch.FloatTensor, values_grad: torch.FloatTensor, module_idx: torch.LongTensor
