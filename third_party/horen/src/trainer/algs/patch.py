...
def _patched_parameters(
    ...
    if getattr(self, "_fast_params", None) is None:
        raise Exception(
            "Tried to get fast weights of a monkey patched module which does " "not encapsulate fast weights."
        )
    ...
    if not self.track_higher_grads and time not in (-1, 0):
        raise ValueError(
            "The patched model is not tracking higher gradients. Only the " "latest parameters are available."
        )
...
def buffer_sync(
    module: _torch.nn.Module, fmodule: _MonkeyPatchBase, device: _typing.Optional[_torch.device] = None
) -> None:
...
def _make_functional(
            ...
            if params is not None and name in params:
                ...
                if isinstance(value, _torch.nn.Module):
                    ...
                    if value is not None:
                        raise TypeError(
                            ("cannot assign '{}' " "as child module '{}'" "(torch.nn.Module or None expected)").format(
                    ...
                    if buffers is not None and name in buffers:
                        if value is not None and not isinstance(value, _torch.Tensor):
                            raise TypeError(
                                ...
                                "(torch.Tensor or None expected)".format(_torch.typename(value), name)
                            )
                        buffers[name] = value
                    else:
    ...
    # Fix for transformers >= 4.40: LlamaModel/Qwen2Model forward uses
    # self.layers[:n] which calls self.__class__(items[slice]), breaking
    # MonkeyPatched.__init__ which requires (original_params, root).
    if issubclass(_ModuleType, _torch.nn.ModuleList):
        def _modulelist_getitem(self, idx):
            items = list(self._modules.values())
            if isinstance(idx, slice):
                return _torch.nn.ModuleList(items[idx])
            return items[idx]
        MonkeyPatched.__getitem__ = _modulelist_getitem
    ...
    for name, child in module._modules.items():
        if child == None:
            continue
        child_params_offset, fchild, _ = _make_functional(child, params_box, child_params_offset, root_patched)
    ...
    def patched_forward(self, *args, params=None, **kwargs):
        ...
        with _modify_internally(self):
            for name, param in zip(self._param_names, params_box[0][params_offset : params_offset + num_params]):
                setattr(self, name, param)

            # This snippet deals with torch.nn.{RNN,GRU,LSTM}
            if hasattr(self, "_flat_weights_names"):
                self._flat_weights = [self._parameters[wn] for wn in self._flat_weights_names]
...
def _update_patched_params(
    fmodule: _MonkeyPatchBase, params_box: _typing.Sequence[_typing.List[_torch.Tensor]], params_offset: int
) -> int:
...
# ==============================================================================
_EncapsulatorType = _typing.Optional[_typing.Callable[[_MonkeyPatchBase, _torch.nn.Module], None]]
...
def make_functional(module: _torch.nn.Module, encapsulator: _EncapsulatorType = None) -> _MonkeyPatchBase:
    ...
    def _refill_params_box(self, params):
        if params is not None:
            ...
            raise ValueError(
                "params keyword must be provided if patched module not " "tracking its own fast parameters"
            )
