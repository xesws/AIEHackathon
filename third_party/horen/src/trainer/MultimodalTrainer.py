    ...
    def edit_step(self, batch, training: bool):
        ...
        with torch.set_grad_enabled(training):
            ...
            l_image_edit = self.model.edit_loss_fn(
                self.config, post_image_edit_logits, post_image_batch_labels, multimodal=True
            )["nll"]
            ...
            if not isinstance(post_base_outputs, torch.Tensor):
                ...
                if kl_mask is None:
                    ...
                    if kl_mask is None:
                        kl_mask = torch.ones(post_base_logits.shape[0], post_base_logits.shape[1]).to(
                            post_base_logits.device
                        )
            else:
        ...
        if self.config.alg == "SERAC_MULTI":
            ...
            l_total_edit = (
                self.config.cedit * l_edit + self.config.cloc * (l_loc + l_image_loc) + self.config.iedit * l_image_edit
            )
