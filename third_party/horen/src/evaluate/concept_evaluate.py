...
def compute_concept_edit_quality(
    ...
    ret = compute_rewrite_or_rephrase_quality(
        model, model_name, hparams, tok, PMT + rewrite_prompts, target_new, device=device, eval_metric=eval_metric
    )
    if test_concept_consistency:
        least_length_gen = 40
        ret["gen_concept_text"] = test_concept_gen(
            model, tok, least_length_gen, PMT + rewrite_prompts, target_new, device=device
        )
