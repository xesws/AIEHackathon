...
def _safe_bleu_like(reference_text: str, hypothesis_text: str) -> float:
    """
    Lightweight BLEU-1 style score with brevity penalty.
    Avoids nltk BLEU incompatibilities on newer Python versions.
    """
    ...
    for tok, cnt in hyp_counts.items():
...
def eval_akew_unstructured(result_path, dataset_type, model_path="all-MiniLM-L6-v2", device=4):
    ...
    for index in tqdm(range(len(data)), desc="Calculate BLEU&ROUGE Score"):
        score = _safe_bleu_like(data[index]["answer"], data[index]["original_prediction"])
        bleu_scores.append(score)

        if dataset_type in ["unke", "counterfact", "wikiupdate", "mquake"]:
            score = _safe_bleu_like(data[index]["answer"], data[index]["para_prediction"])
