...
def compute_sent_metric(
    ...
    same_mask = torch.tensor(
        [i == o for i, o in zip(metric_kwargs["inner_target"], metric_kwargs["all_target"])], device=device
    )
    edit_toks = {
...
def compute_per_metric(
    ...
    with torch.no_grad():
        ...
        result = {
            ...
            "dd": kl_loc_loss(pre=loc_base_logits, post=loc_edit_logits, mask=kl_mask).item(),
        }
