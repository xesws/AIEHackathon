...
def compute_icl_multimodal_edit_quality(
    ...
    if pre_edit:
        edit_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples, target, prompt, image)
    else:
        edit_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples, target, new_fact, image)
    ...
    if "image_rephrase" in record.keys():
        rephrase_image_acc, _ = icl_multimodal_lm_eval(
            model, model_name, hparams, tok, icl_examples, target, new_fact, rephrase_image
        )
    ...
    if "locality_prompt" in record.keys():
        if pre_edit:
            _, _, locality_output = icl_multimodal_lm_eval(
                model, model_name, hparams, tok, icl_examples, loc_a, loc_q, None, is_loc=True
            )
        else:
            _, _, locality_output = icl_multimodal_lm_eval(
                model,
    ...
    if "multimodal_locality_image" in record.keys():
        if pre_edit:
            _, _, locality_image_output = icl_multimodal_lm_eval(
                model, model_name, hparams, tok, icl_examples, m_loc_a, m_loc_q, m_loc_image, is_loc=True
            )
        else:
            _, _, locality_image_output = icl_multimodal_lm_eval(
                model,
...
def icl_multimodal_lm_eval(
    model, model_name, hparams: HyperParams, tokenizer, icl_examples, target, x, image, is_loc=False, neighborhood=False
) -> typing.Dict:
...
def prepare_multimodal_edit(hparams, tok, target, prompts, image):
    ...
    ret = {"text_input": text_input, "image": image, "labels": target, "prompts_len": prompts_len}
    return ret
...
def prepare_multimodal_hf_edit(hparams, processor, target, prompts, image, file_type):
    ...
    if file_type in ["image", "single-image", "multi-image"]:
        multimodal_inputs = processor(images=image, text=text_input, return_tensors="pt").to(
            hparams.device, dtype=hparams.dtype
        )
    elif file_type == "video":
        multimodal_inputs = processor(videos=image, text=text_input, return_tensors="pt").to(
            hparams.device, dtype=hparams.dtype
        )
    elif file_type == "text":
...
def compute_multimodal_edit_results(
    model, model_name, hparams: HyperParams, tok: AutoTokenizer, record: typing.Dict, device
) -> typing.Dict:
    """
    Given a rewritten model, computes generalization and specificity metrics for
    the desired rewrite (passed in via the CounterFact dataset record). Returns a
...
def compute_multimodal_hf_edit_results(
    model, model_name, hparams: HyperParams, tok: AutoProcessor, record: typing.Dict, device
) -> typing.Dict:
    """
    Given a rewritten model, computes generalization and specificity metrics for
    the desired rewrite (passed in via the CounterFact dataset record). Returns a
    ...
    if "multimodal_locality_prompt" in record.keys():
        ...
        m_locality = prepare_multimodal_hf_edit(
            hparams, tok, m_loc_ground_truth, m_loc_prompt, m_loc_image, file_type="image"
        )
