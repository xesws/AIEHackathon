...
class MultimodalEditor:
        ...
        if type(self.model_name) is str:
            if hparams.model_name == "blip2":
                ...
                if hparams is not None and hasattr(hparams, "tokenizer_name"):
                    tok_name = hparams.tokenizer_name if hparams.tokenizer_name is not None else hparams.name
                    tokenizer = getattr(transformers, hparams.tokenizer_class).from_pretrained(tok_name)
    ...
    def edit(
        ...
        **kwargs,
        ...
        requests = self._prepare_requests(
            prompts, targets, image, rephrase_prompts, rephrase_image, locality_inputs, file_type, **kwargs
        )
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                    ...
                    if self.hparams.k != 0:
                        ...
                        metrics = {
                            ...
                            "pre": compute_multimodal_hf_edit_results(
                                self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device
                            ),
                        }
                else:
                ...
                if "locality_output" in metrics["post"].keys():
                    assert len(metrics["post"]["locality_output"]) == len(metrics["pre"]["locality_output"])
                    ...
                    metrics["post"]["locality_acc"] = (
                        sum(post_logits.view(-1) == base_logits.view(-1)) / base_logits.view(-1).shape[0]
                    )
    ...
    def edit_dataset(self, ds: Dataset, keep_original_weight=False, verbose=True, **kwargs):
        # Make Sure dataset supported
        assert (
            sum([isinstance(ds, ds_in_dict) for ds_in_dict in MULTIMODAL_DS_DICT.values()]) > 0
        ...
        for i, request in enumerate(tqdm(ds, desc="Editing dataset", total=len(ds))):
            ...
            if self.alg_name == "IKE":
                ...
                if self.model_name in ["minigpt4", "blip2"]:
                    pre_res = compute_multimodal_edit_results(
                        self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device
                    )
                elif self.model_name in ["llava-onevision", "qwen2-vl"]:
                    pre_res = compute_multimodal_hf_edit_results(
                        self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device
                    )
                edited_model, weights_copy = self.apply_algo(
                    self.model,
            ...
            if self.alg_name == "IKE":
                metrics = {
                    ...
                    "time": exec_time,
                    "post": compute_icl_multimodal_edit_quality(
                        self.model, self.model_name, self.hparams, self.tok, icl_examples, request, self.hparams.device
                    ),
                    "pre": compute_icl_multimodal_edit_quality(
                        self.model,
                        self.model_name,
            ...
            if "locality_output" in metrics["post"].keys():
                ...
                metrics["post"]["locality_acc"] = (
                    sum(post_base_logits_softmax_top_k.view(-1) == base_logits_softmax_top_k.view(-1))
            ...
            if "multimodal_locality_output" in metrics["post"].keys():
                ...
                metrics["post"]["multimodal_locality_acc"] = (
                    sum(post_image_base_logits_softmax_top_k.view(-1) == base_image_logits_softmax_top_k.view(-1))
    ...
    def _prepare_requests(
        ...
        **kwargs,
        ...
        requests = [
            {"prompt": prompt, "target": target, "image": image_, "file_type": file_type_}
        ...
        if "text" in locality_inputs.keys():
            ...
            assert len(locality_inputs["text"]["prompt"]) == len(locality_inputs["text"]["ground_truth"]) == len(
                requests
            ) or print("One Edit instance needs one locality input.....")
        if "vision" in locality_inputs.keys():
            ...
            assert len(locality_inputs["vision"]["prompt"]) == len(locality_inputs["vision"]["ground_truth"]) == len(
                locality_inputs["vision"]["image"]
        ...
        if "text" in locality_inputs.keys():
            ...
            for i, request in enumerate(requests):
                request.update(
                    {"locality_prompt": locality_prompts[i], "locality_ground_truth": locality_ground_truth[i]}
                )
        ...
        if "vision" in locality_inputs.keys():
            ...
            locality_image_path = [
                multimodal_locality_image_ for multimodal_locality_image_ in multimodal_locality_image
            ]
