...
class PerEditor:
    ...
    def edit_dataset(self, ds: Dataset, keep_original_weight=False, verbose=True):
        ...
        assert sum([isinstance(ds, ds_in_dict) for ds_in_dict in PER_DS_DICT.values()]) > 0, print(
            ...
            if self.alg_name == "IKE":
                ...
                metrics.update(
                    compute_per_ike_metric(
                        example=example, model=edited_model, tok=self.tok, device=self.device, test_generation=True
                    )
                )
                if verbose:
                    LOG.info(f"{i} editing: {request['ent']} -> {request['target_personality']}  \n {metrics}")
