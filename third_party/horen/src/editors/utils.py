        ...
        for key in ["locality", "portability"]:
            if key in all_metrics[0][eval].keys() and all_metrics[0][eval][key] != {}:
                ...
                for lkey in get_all_acc_keys(all_metrics):
                    metrics = [
                        np.mean(metric[eval][key][lkey]) for metric in all_metrics if lkey in metric[eval][key].keys()
                    ]
                    if len(metrics) > 0:
...
def summary_metrics_(
    ...
    if not os.path.exists(logs_dir):
        os.makedirs(logs_dir)
    output_file = os.path.join(logs_dir, f"{log_file_prefix}_Checkpoint_{ds_size}_results.json")
    ...
    results = {
        ...
        "exp_time_so_far_s": time() - start_time,
        "total_edit_time_s": total_edit_time,
        "avg_edit_time_s": total_edit_time / ds_size if ds_size > 0 else 0.0,
