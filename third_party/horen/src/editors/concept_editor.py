...
class ConceptEditor:
        ...
        **kwargs,
        ...
        if "requests" in kwargs.keys():
            ...
            requests = self._prepare_requests(
                prompts, target_new, ground_truth, rephrase_prompts, locality_inputs, instance_inputs, **kwargs
            )
        ...
        for i, request in enumerate(requests):
            ...
            if "locality" in all_metrics[i]["post"].keys():
                for locality_key in request["locality"].keys():
                    assert len(all_metrics[i]["post"]["locality"][f"{locality_key}_output"]) == len(
    ...
    def _prepare_requests(
        ...
        **kwargs,
        ...
        requests = [
            {"prompt": prompt, "target_new": target_new_, "ground_truth": ground_truth_, "instance": {}, "locality": {}}
                ...
                for i, request in enumerate(requests):
                    if instance_inputs[instance_key]["prompt"][i] is not None:
                        request["instance"].update(
                            {instance_key: {"prompt": instance_inputs[instance_key]["prompt"][i]}}
                        )
        return requests
