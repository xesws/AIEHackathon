...
class GRACE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        if type(original_layer) is not GRACEAdapter:
            ...
            # values that add_key() had previously made trainable.
...
class GRACEAdapter(torch.nn.Module):
    ...
    def activate_params(self):
        """Restore requires_grad=True on the codebook values after the freeze loop
        in GRACE.__init__ unconditionally sets requires_grad=False on all model
        params. Called on every edit after the first so the optimizer can update
        the existing values in the codebook."""
        if isinstance(self.values, torch.nn.Parameter):
    ...
    def forward(self, *args):
        ...
        if (not self.training) & ("keys" not in self.__dict__):
            ...
            if "keys" not in self.__dict__ or self.keys.nelement() == 0:
                ...
                self.keys, self.values, self.epsilons, self.key_labels, self.edit_ids = self.init_key_value(
                    query, new_value
                )
            elif self.iter == 0:
                ...
                if smallest_distance > (self.init_epsilon + self.epsilons[nearest_key]):
                    ...
                    self.keys, self.values, self.epsilons, self.key_labels, self.edit_ids = self.add_key(
                        query, new_value, self.edit_id
                    )
                else:
