...
class KnowledgeNeurons:
    ...
    def _generate(self, prompt, ground_truth):
        ...
        n_sampling_steps = (
            len(target_label)
            if ("qwen" in self.model_type or "gpt" in self.model_type or "llama" in self.model_type)
            else 1
        )
        all_gt_probs = []
        all_argmax_probs = []
    ...
    def scaled_input(activations: torch.Tensor, steps: int = 20, device: str = "cpu"):
        ...
        if activations.dim() == 2:
            ...
            return tiled_activations * torch.linspace(start=0, end=1, steps=steps).to(tiled_activations.device)[:, None]
    ...
    def get_scores_for_layer(
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                ...
                ) = self.get_baseline_with_activations(encoded_input, layer_idx, mask_idx)
                ...
                for batch_weights in scaled_weights.chunk(n_batches):
                    ...
                    if self.model_type == "t5":
                        ...
                        grad = torch.autograd.grad(torch.unbind(torch.cat(target_probs, dim=0)), batch_weights)[0]
                        grad = grad.sum(dim=0)
