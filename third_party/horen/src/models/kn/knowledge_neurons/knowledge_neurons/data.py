    ...
    if os.path.exists(data_path):
        ...
        for r in tqdm(PARAREL_RELATION_NAMES, "downloading pararel data"):
            with urllib.request.urlopen(
                ...
                graphs = [json.loads(d.strip()) for d in url.read().decode().split("\n") if d]
                PARAREL[r]["graphs"] = graphs
            with urllib.request.urlopen(
                f"https://raw.githubusercontent.com/yanaiela/pararel/main/data/trex_lms_vocab/{r}.jsonl"
            ) as url:
                vocab = [json.loads(d.strip()) for d in url.read().decode().split("\n") if d]
...
def pararel_expanded(data_path: str = "datasets/pararel_expanded.json", obj_label_replacement=None):
    parent_dir = Path(data_path).parent
    os.makedirs(parent_dir, exist_ok=True)
    if os.path.exists(data_path):
        with open(data_path, "r") as f:
        ...
        for key, value in tqdm(PARAREL.items(), "expanding pararel dataset into full sentences"):
            for vocab in value["vocab"]:
                for graph in value["graphs"]:
                    ...
                    full_sentence = sentence.replace("[X]", vocab["sub_label"]).replace("[Y]", "[MASK]")
