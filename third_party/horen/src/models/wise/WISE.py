...
class WISE(torch.nn.Module):
    ...
    def __call__(self, *args, **kwargs):
        if not self.config.retrieve:
            ...
            if hasattr(adapter, "editing") and not adapter.editing:
                if (
                    not adapter.original_layer.weight.equal(adapter.new_weight)
    ...
    def _cal_activation_loss(
        self, original_layer_output, new_weight_layer_output, config=None, act_mask=None, deact_mask=None
    ):
        if hasattr(self.model.config, "batch_size"):
            k = self.config.batch_size
        else:
        ...
        for i, act_mk in enumerate(act_mask):
            if act_mk is not None:
                in_scope_dist = euc(
                    original_layer_output[int(i * len_temp) : int((i + 1) * len_temp), ...],
                ...
                if i == k - 1:
                    out_scope_dist = euc(
                        original_layer_output[int(i - k) :, ...], new_weight_layer_output[int(i - k) :, ...], config
                    )
                else:
                    out_scope_dist = euc(
    ...
    def _cal_memory_pos_activation_loss(
        ...
        return torch.mean(loss4[loss4 > 0]) if min(loss4[loss4 > 0].size()) > 0 else torch.tensor(0.0)
...
class WISEAdapter(torch.nn.Module):
    ...
    def merge_weight(self):
        if self.config.save_freq is not None:  # for ties dare dare_ties
            ...
            merge_alg = merge_dict[self.config.merge_alg]
            cur_new_weight = merge_alg.execute(
                0.5, self.layer.weight, [self.new_weight], densities=self.config.densities
            )
            self.layer.weight = torch.nn.Parameter(cur_new_weight.to(self.layer.weight.device), requires_grad=False)
            self.new_weight = copy.deepcopy(self.original_layer.weight)
    ...
    def new_weight_forward(self, input: Tensor) -> Tensor:
        return (
            ...
            else torch.addmm(self.bias, input.view(-1, input.size(-1)), self.new_weight).view(
