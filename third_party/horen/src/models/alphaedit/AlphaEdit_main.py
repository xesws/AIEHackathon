    ...
    **kwargs,
    ...
    if not os.path.exists(hparams.P_loc):
        ...
        if (
            ...
            raise ValueError(f"Unsupported model for AlphaEdit P initialization: {hparams.model_name}")
        del W_out
...
def execute_AlphaEdit(
    ...
    for i, request in enumerate(requests):
        ...
        print(f"Executing AlphaEdit algo for: " f"[{request['prompt']}] -> [{request['target_new']}]")
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        solve_dtype = P.dtype
        p_i = P[i, :, :].to(f"cuda:{hparams.device}", dtype=solve_dtype)
        cache_i = cache_c[i, :, :].to(f"cuda:{hparams.device}", dtype=solve_dtype)
        layer_ks_cuda = layer_ks.to(f"cuda:{hparams.device}", dtype=solve_dtype)
        resid_cuda = resid.to(f"cuda:{hparams.device}", dtype=solve_dtype)

        upd_matrix = torch.linalg.solve(
            p_i @ (layer_ks_cuda @ layer_ks_cuda.T + cache_i)
            + hparams.L2 * torch.eye(layer_ks.shape[0], dtype=solve_dtype, device=f"cuda:{hparams.device}"),
            p_i @ layer_ks_cuda @ resid_cuda.T,
        )
...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.shape == shape:
        ...
        raise ValueError(
            "Update matrix computed by AlphaEdit does not match original weight shape. " "Check for bugs in the code?"
        )
