...
class DeferAdaptor(torch.nn.Module):
    def __init__(self, config, layer, transpose):
        ...
        self.defer = torch.nn.Linear(input_dim, 1).to(device=self.device, dtype=layer_dtype)
        self.predict_values = torch.nn.Linear(input_dim, output_dim).to(device=self.device, dtype=layer_dtype)
