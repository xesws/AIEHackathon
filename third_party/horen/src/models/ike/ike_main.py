    ...
    **kwargs: Any,
    ...
    if hparams.use_icl_examples is True:
        ...
        query_embedding = util.normalize_embeddings(
            torch.tensor(sentence_model.encode(query_sentence, show_progress_bar=False)).unsqueeze(0).to(device)
        )
...
def apply_ike_to_multimodal_model(
    ...
    **kwargs: Any,
    ...
    safe_model_name = hparams.sentence_model_name.rsplit("/", 1)[-1]
    with open(
        f"{hparams.results_dir}/{hparams.alg_name}/embedding/" f"{hparams.task_name}_embeddings.pkl", "rb"
    ) as fIn:
        stored_data = pickle.load(fIn)
        stored_sentences = stored_data["sentences"]
        stored_embeddings = stored_data["embeddings"]
...
def apply_ike_to_per_model(request, loc_request, tokenizer, device):
    ...
    outer_edit_inputs = [
        edit_icl_prompt_template.format(
            target_per=request["target_personality"], edit_topic=request["ent"], question=question
        )
        + answer
        for question, answer in zip(request["all_prompt"], request["all_comp"])
    ]
    ...
    inner_edit_q = edit_icl_prompt_template.format(
        target_per=request["target_personality"], edit_topic=request["ent"], question=request["inner_prompt"][0]
    )
