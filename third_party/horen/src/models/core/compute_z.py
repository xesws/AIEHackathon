...
def compute_z(
    ...
    if hparams.context is not None:
        context_templates = get_context_templates(
            ...
            tok,
            request,
            context_type=hparams.context,
            n_gen=hparams.ctx_num,
            max_length=hparams.ctx_len,
            ctx_top_k=hparams.ctx_top_k,
        )
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        loss = (
            ...
            + prefix_consistency_loss.to(nll_loss.device)
        )  # 추가
        print(
...
def get_context_templates(model, tok, request, context_type="all", n_gen=5, max_length=10, ctx_top_k: int = 5):
    ...
    if context_type == "all":
        ...
        prompt_key = {"target_true": "ground_truth", "target_new": "target_new", "subject": "subject"}
    ...
    CONTEXT_TEMPLATES_CACHE = [["{}"]] + [
        [
            ...
            for template in generate_fast(
                model, tok, all_prompts, n_gen_per_prompt=n_gen_per_prompt, max_out_len=length, top_k=ctx_top_k
            )
        ]
