...
def execute_core(
    ...
    for request in requests[:10]:
        print(f"CORE request sample: " f"[{request['prompt'].format(request['subject'])}] -> [{request['target_new']}]")
    ...
    context_templates = get_context_templates(
        model=model, tok=tok, length=hparams.ctx_len, ctx_top_k=hparams.ctx_top_k, ctx_num=hparams.ctx_num
    )
...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.shape == shape:
        ...
        raise ValueError(
            "Update matrix computed by CORE does not match original weight shape. " "Check for bugs in the code?"
        )
            ...
            for prompt, n_gen in zip(default_prompts, allocations):
                if n_gen > 0:
                    generated = generate_fast(
                        model, tok, [prompt], n_gen_per_prompt=n_gen, max_out_len=length, top_k=ctx_top_k
                    )
                    templates.extend([f.replace("{", " ").replace("}", " ") + ". {}" for f in generated])
