...
def compute_z(
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        def get_batch_token_indices(tensor: torch.Tensor, batch_idx: int, token_idx: int) -> Tuple[int, int]:
            if batch_idx < tensor.shape[0] and token_idx < tensor.shape[1]:
                return batch_idx, token_idx
            if batch_idx < tensor.shape[1] and token_idx < tensor.shape[0]:
                return token_idx, batch_idx
            raise IndexError(
                f"Cannot index tensor shape={tuple(tensor.shape)} " f"with batch_idx={batch_idx}, token_idx={token_idx}"
            )

        if cur_layer == hparams.layer_module_tmp.format(layer):
            hidden_states = _get_hidden_states(cur_out)

            if target_init is None:
                b_idx, t_idx = get_batch_token_indices(hidden_states, 0, lookup_idxs[0])
                target_init = hidden_states[b_idx, t_idx].detach().clone()

            for i, idx in enumerate(lookup_idxs):
                b_idx, t_idx = get_batch_token_indices(hidden_states, i, idx)
                hidden_states[b_idx, t_idx, :] += delta
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        output = _get_hidden_states(tr[hparams.layer_module_tmp.format(loss_layer)].output)
