...
class UltraEditRewriteExecutor:
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        """
        Apply UltraEdit to the model for one or more requests.

        In sequential editing (one request at a time), set batch_size=1 and
        batch_size_once=1 in the hparams YAML.  The lifelong_normalizer
        accumulates statistics across calls, giving online/sequential behaviour.
        """
        ...
        assert len(requests) >= hparams.batch_size, (
            f"The number of requests ({len(requests)}) must be >= batch_size "
            f"({hparams.batch_size}). Set batch_size: 1 in the YAML for "
            "sequential (one-at-a-time) editing."
        )
        # Take the first batch_size requests
        ...
        for i in range(hparams.batch_size // hparams.batch_size_once):
            ...
            targets = [(" " if request["target_new"][0] != " " else "") + request["target_new"] for request in batch]
            sentences = [request["prompt"] + targets[i] for i, request in enumerate(batch)]
            ...
            # Define labels: mask everything except the target tokens
        ...
        # Sort by sequence length (longest first) for efficiency
        batchs = sorted(batchs, key=lambda x: torch.sum(x["attention_mask"]).item(), reverse=True)
