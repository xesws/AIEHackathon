...
class Variance(Stat):
    ...
    def variance(self, unbiased=True):
        return self._restore_result_shape(self.v_cmom2 / (self.count - (1 if unbiased else 0)))
...
class Covariance(Stat):
    ...
    def covariance(self, unbiased=True):
        return self._restore_result_shape(self.cmom2 / (self.count - (1 if unbiased else 0)))
    ...
    def variance(self, unbiased=True):
        return self._restore_result_shape(self.cmom2.diag() / (self.count - (1 if unbiased else 0)))
...
class CrossCovariance(Stat):
    ...
    def add(self, a, b):
        ...
        if len(a.shape) > 2:
            a, b = [d.view(d.shape[0], d.shape[1], -1).permute(0, 2, 1).reshape(-1, d.shape[1]) for d in [a, b]]
        batch_count = a.shape[0]
        # Initial batch.
        if self._mean is None:
            self.count = batch_count
...
class Quantile(Stat):
    ...
    def _lazy_init(self, incoming):
        ...
        self.data = [torch.zeros(self.depth, self.resolution, dtype=self.dtype, device=self.device)]
    ...
    def _add_every(self, incoming):
        ...
        while index < supplied:
            ...
            self.data[0][:, ff : ff + copycount] = torch.t(incoming[index : index + copycount, :])
    ...
    def _scan_extremes(self, incoming):
        ...
        self._update_extremes(torch.min(incoming, dim=0)[0], torch.max(incoming, dim=0)[0])

    def _update_extremes(self, minr, maxr):
        self.extremes[:, 0] = torch.min(torch.stack([self.extremes[:, 0], minr]), dim=0)[0]
        self.extremes[:, -1] = torch.max(torch.stack([self.extremes[:, -1], maxr]), dim=0)[0]
    ...
    def variance(self, unbiased=True):
        mean = self.mean()[:, None]
        return self.integrate(lambda x: (x - mean).pow(2)) / (self.count - (1 if unbiased else 0))
    ...
    def _expand(self):
        ...
        if cap > 0:
            ...
            self.data.insert(0, torch.zeros(self.depth, cap, dtype=self.dtype, device=self.device))
        ...
        for index in range(1, len(self.data)):
            ...
            if self.data[index - 1].shape[1] - (amount + position) >= (
                ...
                self.data[index - 1][:, position : position + amount] = self.data[index][:, :amount]
    ...
    def _weighted_summary(self, sort=True):
        ...
        if sort:
            ...
            summary = torch.cat([self.extremes[:, :1], summary, self.extremes[:, 1:]], dim=-1)
            weights = torch.cat(
                [
    ...
    def quantiles(self, quantiles):
        ...
        result = torch.zeros(self.depth, quantiles.numel(), dtype=self.dtype, device=self.device)
        ...
        for d in range(self.depth):
            result[d] = torch.tensor(numpy.interp(nq, ncw[d], nsm[d]), dtype=self.dtype, device=self.device)
...
def sample_portion(vec, p=0.5):
    ...
    """
    bits = torch.bernoulli(torch.zeros(vec.shape[0], dtype=torch.uint8, device=vec.device), p)
    return vec[bits]
...
class TopK:
    ...
    def add(self, data, index=None):
        ...
        if self.top_data is None:
            ...
            self.top_data = torch.zeros(feature_size, max(10, self.k * 5), out=data.new())
            self.top_index = self.top_data.clone().long()
            self.linear_index = (
                0
                if len(data.shape) == 1
                else torch.arange(feature_size, out=self.top_index.new()).mul_(self.top_data.shape[-1])[:, None]
            )
        size = data.shape[0]
        sk = min(size, self.k)
        if self.top_data.shape[-1] < self.next + sk:
            # Compression: if full, keep topk only.
            self.top_data[:, : self.k], self.top_index[:, : self.k] = self.topk(sorted=False, flat=True)
            self.next = self.k
    ...
    def topk(self, sorted=True, flat=False):
        ...
        td, bti = self.top_data[:, : self.next].topk(k, sorted=sorted, largest=self.largest)
        # we want to report top indexes globally, which is ti.
        ti = self.top_index.view(-1)[(bti + self.linear_index).view(-1)].view(*bti.shape)
        if flat:
            return td, ti
        else:
            return (
    ...
    def state_dict(self):
        return dict(
            ...
            next=self.next,
            linear_index=(
                self.linear_index.cpu().numpy() if isinstance(self.linear_index, torch.Tensor) else self.linear_index
            ),
            perm=self.perm,
        )
...
class History(Stat):
    ...
    def _cat_added(self):
        if len(self._added):
            self._data = torch.cat(([self._data] if self._data is not None else []) + self._added)
            self._added = []
...
null_numpy_value = numpy.array(struct.unpack(">d", struct.pack(">Q", 0xFFF8000000000002))[0], dtype=numpy.float64)
...
class FixedRandomSubsetSampler(FixedSubsetSampler):
    ...
    def class_subset(self, class_filter):
        ...
        if isinstance(class_filter, int):
            ...
            rule = class_filter
        return self.subset([i for i, j in enumerate(self.samples) if rule(self.data_source[j])])


def make_loader(dataset, sample_size=None, batch_size=1, sampler=None, random_sample=None, **kwargs):
    ...
    if sample_size is not None:
        ...
        if sample_size > len(dataset):
            print("Warning: sample size %d > dataset size %d" % (sample_size, len(dataset)))
            sample_size = len(dataset)
        if random_sample is None:
            sampler = FixedSubsetSampler(list(range(sample_size)))
        else:
            sampler = FixedRandomSubsetSampler(dataset, seed=random_sample, end=sample_size)
    return torch.utils.data.DataLoader(dataset, sampler=sampler, batch_size=batch_size, **kwargs)
...
def _unit_test():
    ...
    # Test Covariance
    goal = torch.tensor(numpy.random.RandomState(1).standard_normal(10 * 10)).view(10, 10)
    data = (
        torch.tensor(numpy.random.RandomState(2).standard_normal(args.test_size * 10)).view(args.test_size, 10).mm(goal)
    )
    ...
    ds = torch.utils.data.TensorDataset(
        (torch.arange(args.test_size)[:, None] % torch.arange(1, 6)[None, :] == 0).double(),
        (torch.arange(args.test_size)[:, None] % torch.arange(5, 8)[None, :] == 0).double(),
    )
    ...
    minerr = torch.max(torch.abs(qc.minmax().cpu()[:, 0] - torch.arange(qc.depth, dtype=torch.float) * amount))
    maxerr = torch.max(
        torch.abs((qc.minmax().cpu()[:, -1] + 1) - (torch.arange(qc.depth, dtype=torch.float) + 1) * amount)
    )
    print("Minmax error %f, %f" % (minerr, maxerr))
    interr = torch.max(torch.abs(qc.integrate(lambda x: x * x).cpu() - actual_sum) / actual_sum)
    print("Integral error: %f" % interr)
    medianerr = torch.max(torch.abs(qc.median() - alldata.median(0)[0]) / alldata.median(0)[0]).cpu()
    ...
    counterr = ((qc.integrate(lambda x: torch.ones(x.shape[-1]).cpu()) - qc.size()) / (0.0 + qc.size())).item()
