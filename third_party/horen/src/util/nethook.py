...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        def retain_hook(m, inputs, output_or_kwargs, maybe_output=None):
            # PyTorch hook signatures:
            # - without kwargs: (module, args, output)
            # - with kwargs:    (module, args, kwargs, output)
            if maybe_output is None:
                kwargs = None
                output = output_or_kwargs
            else:
                kwargs = output_or_kwargs
                output = maybe_output
...
def invoke_with_optional_args(fn, *args, **kwargs):
    ...
    pass_kw = {
        k: v for k, v in kwargs.items() if k not in used_kw and (k in argspec.kwonlyargs or argspec.varargs is not None)
    }
