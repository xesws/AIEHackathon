...
from .inference_counter import increment_generation_call_count
...
def generate_interactive(
    ...
    while True:
        ...
        if use_logit_lens:
            inp_prompt = tok([prompt], padding=True, return_tensors="pt").to(next(model.parameters()).device)
...
def generate_fast(
    ...
    if vanilla_generation:
        ...
        gen_txt = model.generate(input_ids=input_ids, attention_mask=attention_mask, max_new_tokens=max_out_len)
...
def generate_standard(
    ...
    inputs = tokenizer(prompt, padding=True, truncation=True, max_length=max_input_tokens, return_tensors="pt").to(
        "cuda"
    )
